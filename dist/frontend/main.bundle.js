...
}`, "",{"version":3,"sources":["webpack://./src/frontend/style.css"],"names":[],"mappings":"AAAA;IACI,sBAAsB;IACtB,kBAAkB;IAClB,iCAAiC;IACjC,aAAa;AACjB;AACA;IACI,qBAAqB;AACzB","sourcesContent":[".gameItem{\r\n    border: 1px solid #ddd;\r\n    border-radius: 5px;\r\n    box-shadow: 5px 10px 10px #e8f5ee;\r\n    height: 100px;\r\n}\r\n.unmatched{\r\n    background-color: red;\r\n}"],"sourceRoot":""}]);
// Exports
...
class GameController {
    ...
    constructor(items, element) {
        ...
        this.items = [];
        this.initGame(items);
        this.startTime = Date.now(); // Thời gian bắt đầu đếm ngược
        this.timerInterval = setInterval(this.updateTimer.bind(this), 1000); // Cập nhật thời gian mỗi giây
    }
    initGame(initData) {
    ...
    isMatched(id, imgElement) {
        let openItems = this.items.filter((item) => {
            return item.status === _models_GameItem__WEBPACK_IMPORTED_MODULE_1__.GameItemStatus.Open && !item.isMathched;
        });
    ...
    changeMatchedBackground(imgElement, isMatched = true) {
        if (!(imgElement === null || imgElement === void 0 ? void 0 : imgElement.parentElement))
            return; // Kiểm tra imgElement có tồn tại
        if (isMatched) {
    ...
    updateTimer() {
        ...
        const elapsedTime = Math.floor((currentTime - this.startTime) / 1000); // Convert milliseconds to seconds
    
        const countdown = 120 - elapsedTime; // Countdown time is 120 seconds (2 minutes)
        ...
        if (countdown <= 0) {
            clearInterval(this.timerInterval); // Stop the countdown when time is up
            this.endGame();
        } else if (this.isWinGame()) {
            clearInterval(this.timerInterval); // Stop the countdown if the game is won before time is up
            alert("Bạn đã chiến thắng"); // Display the winning message
            this.processResetButtonClicked(); // Reset the game
        }
    }
    
    
    endGame() {
        // Thêm code xử lý khi kết thúc trò chơi ở đây
        alert("Bạn đã thua trò chơi");
    }
}
__decorate([
    autobind_decorator__WEBPACK_IMPORTED_MODULE_0__["default"]
], GameController.prototype, "processGameItemClick", null);
__decorate([
    autobind_decorator__WEBPACK_IMPORTED_MODULE_0__["default"]
...
class GameItem {
    constructor(id, divId, image, status = GameItemStatus.Close, isMathched = false, imageElement = null) {
        this.id = id;
        this.divId = divId;
        this.image = image;
        this.status = status;
    ...
    renderResetButton(rootElement) {
        let button = rootElement.querySelector('button#reset');
        if (button) {
            button.addEventListener('click', this.processResetButtonClicked); // Thêm sự kiện click cho nút reset
        }
    }
...
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["vendors-node_modules_autobind-decorator_lib_esm_index_js-node_modules_css-loader_dist_runtime-efc508"], () => (__webpack_require__("./src/frontend/app.ts")))
